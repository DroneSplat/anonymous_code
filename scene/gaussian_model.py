...
class GaussianModel:
    ...
    def __init__(self, sh_degree : int, use_hooks: bool = True, optim_pose = False):
    ...
    def compute_relative_world_to_camera(self, R1, t1, R2, t2):
        ...
        E1_inv = np.hstack([R1.T, -R1.T @ t1.reshape(-1, 1)]) 
        E1_inv = np.vstack([E1_inv, zero_row]) 

        E2 = np.hstack([R2, -R2 @ t2.reshape(-1, 1)])  
        E2 = np.vstack([E2, zero_row]) 
    ...
    def init_RT_seq(self, cam_list):
        ...
        if self.optim_pose:
            ...
            self.P = poses.cuda().requires_grad_(False)
    ...
    def init_RT_seq_test(self, cam_list):
        ...
        for cam in cam_list[1.0]:
            ...
            poses.append(p)
        poses = torch.stack(poses)
        self.test_P = poses.cuda().requires_grad_(False)
    ...
    def create_from_pcd(self, pcd : BasicPointCloud, spatial_lr_scale : float):
        ...
        if self.use_hooks:
            self._scaling.register_hook(self.scale_gradient_hook)
            self._xyz.register_hook(self.xyz_gradient_hook)
    ...
    def prune_points(self, mask):
        ...
        self._normal = self._normal[valid_points_mask]  # 更新normal属性
        if self.use_hooks:
            self._scaling.register_hook(self.scale_gradient_hook)
            self._xyz.register_hook(self.xyz_gradient_hook)
    ...
    def densify_and_split(self, grads, grad_threshold, scene_extent, N=2):
        ...
        new_normal = self._normal[selected_pts_mask].repeat(N, 1)  # 复制normal属性
